...
from fastapi import APIRouter, Body, Request, Response, HTTPException, status, UploadFile, Form
...
def create_subject(request: Request, subject: Subject = Form()):
    subject = form_change_to_json(subject)
    subject = {
        "subject_name": subject.subject,
        "gender": subject.gender,
        "age": subject.age
    }
    new_subject = request.app.db.subject.insert_one(subject)
...
#列出測試者所有相關資料
@router.get("/list/data",response_model=Subject, response_description="list all data about subject",status_code=status.HTTP_200_OK)
def list_data(user_id: str, request: Request):
    if (subject := request.app.db.subject.find_one({"_id": ObjectId(user_id)})) is not None:
        return subject
...
#修改影片
@router.post("/update/video",response_model = Video, response_description="do some change in video",status_code=status.HTTP_200_OK)
...
#刪除影片紀錄
@router.delete("/delete/video", response_description="Delete a video",status_code=status.HTTP_200_OK)
